...
/**
 * OKR Agent Integration Test Harness (WebSocket)
 *
...
async function runTest() {
  log('\n========================================', colors.cyan);
  log('OKR Agent WebSocket Integration Test', colors.cyan);
  log('========================================\n', colors.cyan);
  ...
  try {
    ...
    log('\n--- Test 2: WebSocket Connection ---', colors.blue);
    ...
    await new Promise<void>((resolve, reject) => {
      ws.on('open', () => {
        ...
        logTest('WebSocket Connect', true, 'Connected successfully', duration);
        ...
        ws.once('message', (data) => {
          try {
            ...
            if (response.type === 'joined_session') {
              logTest('Join Session', true, `Joined session ${sessionId}`, Date.now() - wsStartTime);
              resolve();
            }
          } catch (error: any) {
            reject(error);
      ...
      ws.on('error', (error) => {
        const duration = Date.now() - wsStartTime;
        logTest('WebSocket Connect', false, error.message, duration);
        reject(error);
      });
    });
    ...
    await sendWebSocketMessage(ws, sessionId, msg1, 'Initial Message');
    await sleep(2000);
    ...
    await sendWebSocketMessage(ws, sessionId, msg2, 'Stakeholder Info');
    await sleep(2000);
    ...
    await sendWebSocketMessage(ws, sessionId, msg3, 'Problem Context');
    await sleep(2000);
    ...
    await sendWebSocketMessage(ws, sessionId, msg4, 'Success Metrics');
    await sleep(2000);
    ...
    await sendWebSocketMessage(ws, sessionId, msg5, 'Confirm Objective');
